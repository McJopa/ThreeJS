...
ThreeJSQueue.prototype.enqueueAnimate = function (
  this: ThreeJSQueue,
  name: string,
  objectRef: Mesh,
  dimensions: Vector3
  ...
  const pos = new Vector3(
    -this.queuePosLength + this.posOffset,
    this.groundPos + dimensions.y / 2,
    0
  );
...
ThreeJSQueue.prototype.dequeueAnimate = function (this: ThreeJSQueue): void {
  ...
  this.movementAnimations.push({
    ...
    desiredPos: new Vector3(
      data.objectRef.position.x + 30,
      data.objectRef.position.y,
      data.objectRef.position.z
    ),
    velocity: new Vector3(0, 0, 0),
  });
  ...
  this.queue.forEach((obj: queueData) => {
    ...
    const movementData = {
      ...
      desiredPos: new Vector3(
        newXPos,
        obj.objectRef.position.y,
        obj.objectRef.position.z
      ),
      velocity: new Vector3(0, 0, 0),
    };
    // move(movementData);
...
ThreeJSQueue.prototype.animateMovements = function (this: ThreeJSQueue): void {
  ...
  this.movementAnimations = this.movementAnimations.filter(
    (obj: movementData) => !obj.curPos.equals(obj.desiredPos)
  );
...
function checkWithinDistanceThreshold(movementData: movementData) {
  ...
  if (
    Math.abs(movementData.curPos.x - movementData.desiredPos.x) <
      distanceThreshold &&
    Math.abs(movementData.curPos.y - movementData.desiredPos.y) <
      distanceThreshold &&
    Math.abs(movementData.curPos.z - movementData.desiredPos.z) <
      distanceThreshold
  ) {
